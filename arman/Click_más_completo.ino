...
// Serial.println(estadoBoton);
  ...
  int estadoEjeX = analogRead(A1);
 // Serial.println(estadoEjeY);
  int espera = map(estadoEjeY, 0, 1023, 50, 270);    
 // Serial.println(espera);
  valorCromatico = map (estadoEjeX, 0 , 1023, 0 , 255);
  tiempoActual = millis();
//Serial.println(tiempoActual);
  if (tiempoActual> (tiempoAnteriorQueActualizamosElLed + espera)) {
    tiempoAnteriorQueActualizamosElLed = tiempoActual;
    if (ledEncendido) {
        leds[ledNave] = CHSV(valorCromatico, 255, 255); 
       // valorCromatico++;
     //   if (valorCromatico > 255) { valorCromatico = 0; }
        Serial.println(valorCromatico);
         FastLED.show();      
    }
    else {
  ...
  if(estadoBoton == HIGH && estadoBotonAnterior!= estadoBoton ) {
    contadorClicks ++;
  ...
  if ( contadorClicks == objetivoClicks){
    ...
    ledNave +=1;
    contadorClicks = 0;
    objetivoClicks = random(1,4); //de una a tres veces
  }
  if (ledNave > 178) {
    ledNave = 0;
  }
